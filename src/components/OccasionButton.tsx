...
export function OccasionButton({ button, onPress, isPlaying }: Props) {
  ...
  return (
    <button
      onClick={onPress}
      className="relative flex items-center justify-center w-full h-full min-h-[80px] rounded-xl select-none touch-manipulation transition-transform duration-75 active:scale-95 focus:outline-none focus:ring-2 focus:ring-white/30"
      style={{
    ...
    >
      ...
      {isPlaying && (
        <span className="absolute inset-0 rounded-xl ring-2 ring-white/80" />
      )}
